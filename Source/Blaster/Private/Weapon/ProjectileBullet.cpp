...
void AProjectileBullet::OnHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit) {
	 ...
	 * FDamageEvent general struct is just to calculate ActualDamage amount from given Damage
	...
	if (OtherActor && (Damage > 0.f)) {
		...
		/* must broadcast controller's delegate
		 * controller has delegate, to know if that controller had instigated any damage */
